...
{{-- @include('backend.includes.likes',['likes'=>$row->likes,'counter'=>$row->likes->count()])
@include('backend.includes.dislikes',['dislikes'=>$row->dislikes,'counter'=>$row->dislikes->count()]) --}}



@include ('comments.list', ['collection' => $row->comments['root']])

{{-- @include ('backend.partials.comments.list', ['collection' => $row->getThreadedComments]) --}}
