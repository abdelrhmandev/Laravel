...
!(function (e) {
    e.fn.imageUploader = function (t) {
        ...
        let o = function () {
            ...
            g = function (e) {
                FileExtensionsMsg = `The file "${e.name}" does not match with the accepted file extensions: "${a.settings.extensions.join('", "')}"`;
                return !(a.settings.extensions.indexOf(e.name.replace(new RegExp("^.*\\."), ".")) < 0) || (createCustomAlert(FileExtensionsMsg), !1);
            },
            c = function (e) {
                FileMimeTypesMsg = `The file "${e.name}" does not match with the accepted mime types: "${a.settings.mimes.join('", "')}"`;
                return !(a.settings.mimes.indexOf(e.type) < 0) || (createCustomAlert(FileMimeTypesMsg), !1);
            },
            f = function (e) {
                FileMaxsizeMsg = `The file "${e.name}" exceeds the maximum size of ${a.settings.maxSize / 1024 / 1024}Mb`;
                return !(e.size > a.settings.maxSize) || (createCustomAlert(FileMaxsizeMsg), !1);
            },
            m = function (e, t) {
                FilelimitMsg = `The file "${t.name}" could not be added because the limit of ${a.settings.maxFiles} files was reached`;
                return !(e + s.items.length + a.settings.preloaded.length >= a.settings.maxFiles) || (createCustomAlert(FilelimitMsg), !1);
            },
            u = function (t, n) {
