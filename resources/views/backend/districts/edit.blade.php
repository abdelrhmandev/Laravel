...
@section('breadcrumbs')
    ...
    <ul class="breadcrumb breadcrumb-separatorless fw-semibold fs-7 my-1">
        <li class="breadcrumb-item text-muted"><a href="{{ route(config('custom.route_prefix') . '.dashboard') }}"
                    ...
                    <div class="card-body pt-0">
                        <div class="d-flex flex-column gap-5 mt-2">
                            ...
                            <x-backend.langs.LangInputs :showDescription="0" :richTextArea="0" :showSlug="1"
                                :row="$row" :columnvalues="$TrsanslatedColumnValues" />
    ...
    <script>
        ...
        $.ajax({
            ...
            success: function(data) {
                ...
                $.each(data, function(key, value) {
                    $('select[name="city_id"]').append('<option value="' + key + '">' + value +
                        '</option>');
                });
            }
        });
            ...
            if (country_id > 0) {
                ...
                $.ajax({
                    ...
                    success: function(data) {
                        $('select[name="city_id"]').empty();
                        $.each(data, function(key, value) {
                            $('select[name="city_id"]').append('<option value="' + key + '">' +
                                value + '</option>');
                        });
                    }
                });
        ...
        $.ajax({
            ...
            success: function(data) {
                ...
                $.each(data, function(key, value) {
                    $('select[name="area_id"]').append('<option value="' + key + '">' + value +
                        '</option>');
                });
            }
        });
            ...
            if (city_id > 0) {
                ...
                $.ajax({
                    ...
                    success: function(data) {
                        $('select[name="area_id"]').empty();
                        $.each(data, function(key, value) {
                            $('select[name="area_id"]').append('<option value="' + key + '">' +
                                value + '</option>');
                        });
                    }
                });
